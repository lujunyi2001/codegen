...
public class OracleColumnSelector extends ColumnSelector {
	...
	protected String getColumnInfoSQL(String tableName) {
		...
		sb.append(" utc.DATA_PRECISION as precision,utc.DATA_SCALE scale,utc.data_length 最大长度, ");
	...
	protected ColumnDefinition buildColumnDefinition(Map<String, Object> rowMap){
		...
		if(precision != null) {
			columnDefinition.setNumberPrecision(precision.intValue());
		}
		
		columnDefinition.setComment((String)rowMap.get("COMMENTS"));
		
		BigDecimal scale = (BigDecimal) rowMap.get("scale");
		if(scale != null) {
			columnDefinition.setNumberScale(scale.intValue());
		}
