...
export default {
  ...
  methods: {
    onGenerate() {
      debugger
      console.log(this.clientParam)
      if (!this.clientParam.tableDefinitionIds || this.clientParam.tableDefinitionIds.length === 0) {
        this.tip('请勾选表', 'error')
        return
      }
      if (!this.clientParam.templateConfigIdList || this.clientParam.templateConfigIdList.length === 0) {
        this.tip('请勾选模板', 'error')
        return
      }
      this.post('/generate/codeFromDefinition', this.clientParam, resp => {
        this.loading = false
