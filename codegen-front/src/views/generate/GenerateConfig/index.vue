  ...
  <div class="code-gen">
    ...
    <el-row v-show="showTable" :gutter="20">
      ...
      <el-col id="templateSelect" :span="12">
        ...
        <el-button v-show="showTable" type="primary" @click="onGenerateTableDefinition">生成表定义</el-button>
      </el-col>
    </el-row>
    ...
    >
      ...
      >
        ...
        <el-form-item label="包名" prop="packageNamePrefix">
...
export default {
  ...
  methods: {
    ...
    onDataSourceAdd() {
      ...
      this.datasourceFormData.id = null
    ...
    onDataSourceChange(datasourceConfigId) {
      ...
      this.datasourceConfigList.find((item) => {
        if (item.id === datasourceConfigId) {
          this.clientParam.packageNamePrefix = item.packageNamePrefix
    ...
    onGenerateTableDefinition() {
      this.$refs.genForm.validate((valid) => {
        if (valid) {
          ...
          const that = this
          this.post('/generate/generateTableDefinition', this.clientParam, resp => {
            that.$confirm('生成表定义成功,是否跳转查看?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning',
              beforeClose: (action, instance, done) => {
                if (action === 'confirm') {
                  const config = this.clientParam.datasourceConfigId
                  this.goRoute(`/generateDefinition/list/${config}`)
                }
                done()
              }
            }).catch(function() {})
